...
describe("Claude Integration Tests", () => {
  ...
  describe("Successful Claude execution", () => {
    test("should execute Claude with valid part_1 content and stream output", async () => {
      ...
      process.nextTick(() => {
        mockProcess.stdout.emit(
          "data",
          '{"type":"assistant","message":{"content":[{"type":"text","text":"Streaming progress"}]}}\n'
        );
        mockProcess.stdout.emit(
          "data",
          '{"type":"result","result":"Summarized content"}\n'
        );
      ...
      expect(mockProcess.stdin.write).toHaveBeenCalledWith(
        "<text_to_summarize>" + content + "</text_to_summarize>"
      );
    ...
    test("should handle different model parameters", async () => {
      ...
      process.nextTick(() => {
        mockProcess.stdout.emit(
          "data",
          '{"type":"assistant","message":{"content":[{"type":"text","text":"Streaming with opus"}]}}\n'
        );
        mockProcess.stdout.emit(
          "data",
          '{"type":"result","result":"Summarized with opus"}\n'
        );
        mockProcess.emit("close", 0);
      });
    ...
    test("should handle multiline output and stream it", async () => {
      ...
      process.nextTick(() => {
        mockProcess.stdout.emit(
          "data",
          '{"type":"assistant","message":{"content":[{"type":"text","text":"Streaming multiline"}]}}\n'
        );
        mockProcess.stdout.emit(
          "data",
  ...
  describe("Error handling", () => {
    ...
    test("should handle process exit with error code", async () => {
      ...
      await expect(summaryPromise).rejects.toThrow(
        "Claude process exited with code 1: Claude error message"
      );
    });
  ...
  describe("Input validation", () => {
    test("should handle null content", async () => {
      await expect(summarizeWithClaude(null)).rejects.toThrow(
        "Content cannot be null or undefined"
      );
    });

    test("should handle undefined content", async () => {
      await expect(summarizeWithClaude(undefined)).rejects.toThrow(
        "Content cannot be null or undefined"
      );
    });

    test("should handle empty string content", async () => {
      await expect(summarizeWithClaude("")).rejects.toThrow(
